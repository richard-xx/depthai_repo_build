...
build_package() {
    ...
    *)
    ...
    if [[ "${package_type}" == "core" ]] || [[ "${ROS_DISTRO}" == "kinetic" ]] || [[ "${ROS_DISTRO}" == "melodic" ]] || [[ "${LINUX_DISTRO}" == "buster" ]]; then
        ...
        env DEB_BUILD_OPTIONS=noautodbgsym BUILD_TESTING_ARG="-DOpenCV_DIR=/home/ubuntu/OpenCV4.2/lib/cmake/opencv4" dpkg-buildpackage -b -j"$(nproc)" -Zgzip # 使用BUILD_TESTING_ARG和OpenCV_DIR构建二进制包
    else
        info "Building ${ros_pkg_name} binary package..."                    # 提示正在构建二进制包
        env DEB_BUILD_OPTIONS=noautodbgsym dpkg-buildpackage -b -j"$(nproc)" # 构建二进制包
    fi
